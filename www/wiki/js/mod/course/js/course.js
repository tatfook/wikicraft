...
define(['app', 'helper/util', 
        ...
        'text!wikimod/course/html/catalog.html'], function (app, util, htmlContent, _, catalog) {

    function registerController(wikiBlock) {
        app.registerController("courseController", ['$scope', '$uibModal', '$timeout', '$http', function ($scope, $uibModal, $timeout, $http) {

            $scope.host = 'http://121.14.117.239/api/lecture/course';

            function init() {

                $http.post($scope.host + '/courselist', {
                    course_url: $scope.pageinfo.url || window.location.pathname,
                }, {
                    isShowLoading: true
                }).then(function (rs) {
                    var data = rs.data;
                    ...
                    if (data && data.err === 0) {
                        ...
                        $scope.course.title = data.course;

                        $scope.action.switch = parseInt(data.course.is_stage, 10) === 1 ? true : false;
                        ...
                        $scope.course.chapter = items;

                        // setTimeout(function() {
                        //     var ele = $('.chapter-no-stage>.chapter').get();
            ...
            $scope.item = {
                ...
                setItem: function (isStage, title, chapter_url, stage) {
                    ...
                    // if(isStage){    //阶段下的列表修改
                    //     that.stage[stage].title = title;
                    //     that.stage[stage].chapter_url = chapter_url;
                    // }else{  //非阶段的列表修改
                    //     that.no_stage.title = title;
                    //     that.no_stage.chapter_url = chapter_url;
                    // }
                }
            };
            ...
            $scope.course = {
                ...
                //判断是否有数据，用于一开始显示列表
                hasData: function (e) {
                    return $scope.course.title === null || angular.equals({}, $scope.course.title) ? true : false;
                }
            }
            ...
            $scope.action = {
                ...
                cancelUpdateItem: function (idx, newItem) {
                    ...
                    newItem["chapter_url"] = oldItem["chapter_url"];
            ...
            $scope.save = function () {
                ...
                item = angular.copy($scope.course.chapter),
                    ...
                    is_stage = $scope.action.switch ? 1 : 0;
            ...
            $scope.viewCourseEditor = function () {
            ...
            $scope.catalog = {
                ...
                toggleChange: function (type, idx) {
                    var that = $scope.catalog,
                        item = that.item;

                    if (type === 'all') {
                        that.state = !that.state;
                        var i = 0,
                            obj = angular.equals({}, that.item) ? $scope.course.chapterWithStage : item;
    ...
    return {
        render: function (wikiBlock) {
            ...
            // return '<div ng-controller="courseController" ng-click="viewCourseEditor()" style="min-height: 100px"><div ng-bind-html="htmlContent"></div></div>';
            return `<div ng-controller="courseController" ng-click="viewCourseEditor();" style="min-height: 100px; cursor:pointer;">
                        <div ng-show = 'course.hasData()'>点击编辑课程内容^-^</div>
                        <div ng-hide = 'course.hasData()' ng-class="{true: 'disabled', false: '' }[isDisabled]"> ` + catalog + ` </div>
                    </div>`
        }
    }
});