...
define([
  ...
  app.registerController('payController', [
    '$scope',
    'Account',
    'modal',
    '$rootScope',
    '$http',
    function($scope, Account, modal, $rootScope, $http) {
      function init() {
        ...
        validateF(
          { app_goods_id: queryArgs.app_goods_id },
          '$scope.app_goods_id'
        )
        ...
        $scope.wechatClient = function() {
          ...
          if (
            confirm(
              '使用微信H5支付，必须安装微信后才能继续，请问是否安装了微信客户端？'
            )
          ) {
            createCharge(params, function(charge) {
        ...
        $scope.alipayQR = function() {
          ...
          createCharge(params, function(charge) {
            ...
            $scope.alipayNotice = 'a'
            getTrade(charge)
            getQRCode('.qrcode-alipay', 166, 166)
          })
        }
        ...
        $scope.wechatQR = function() {
          ...
          createCharge(params, function(charge) {
            ...
            $scope.page = 'wechat'
            getTrade(charge)
            getQRCode('.qrcode-wechat', 200, 200)
          })
        }
        ...
        $scope.$watch(
          ...
          function(newValue, oldValue) {
            ...
            if (
              newValue &&
              newValue.price &&
              reg.test(newValue.price) &&
              typeof newValue.exchange_rate == 'number'
            ) {
        ...
        function createCharge(params, callback) {
          ...
          util.http(
            'POST',
            config.apiUrlPrefix + 'pay/createCharge',
            params,
            function(response) {
        ...
        function getAppGoodsInfo() {
          var params = {
            app_goods_id: $scope.app_goods_id,
            app_name: $scope.app_name
          }

          util.http(
            'POST',
            config.apiUrlPrefix + 'goods/getAppGoodsInfo',
            params,
            function(response) {
              bAppExist = true
        ...
        function getUserinfo() {
          $http
            .post(
              config.apiUrlPrefix + 'user/getBaseInfoByName',
              { username: $scope.otherUserinfo.username },
              { isShowLoading: false }
            )
            .then(
              function(response) {
        ...
        function getTrade(charge) {
          $http
            .post(
              config.apiUrlPrefix + 'pay/getTradeOne',
              {
                username: $scope.otherUserinfo.username,
                trade_no: charge.order_no
              },
              { isShowLoading: false }
            )
            .then(function(response) {
              if (response && response.status) {
                if (
                  ...
                  response.data &&
                  response.data.data &&
                  response.data.data.status == 'Finish'
                ) {
                  $scope.page = 'success'
                  if ($scope.returnUrl) {
                    setTimeout(function() {
        ...
        function getQRCode(name, width, height) {
          setTimeout(function() {
        ...
        $http({
          method: 'GET',
          url:
            config.keepworkOfficialGitHost +
            '/gitlab_rls_official/keepworkhaqi/raw/master/official/haqi/payads.md',
          headers: {
