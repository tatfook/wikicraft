...
define([
	...
	function getCurrentSite(username, sitename) {
		if (!util.isEmptyObject(currentPage)) {
    ...
    app.registerController('wikiEditorController', [
        '$scope',
        '$rootScope',
        '$location',
        '$http',
        ...
        function (
            ...
            modal) {
                ...
                $(window).on("beforeunload", function (e) {
                    if (currentPage && currentPage.isModify) {
                    ...
                    fnList.push(function(finish){
                        if ($scope.user && $scope.user.username) {
                            ...
                            util.post(config.apiUrlPrefix + "site_user/getSiteListByMemberName", {
                                ...
                                data = data || [];
                                for (var i = 0; i < data.length; i++) {
                                    if (util.isEmptyObject(data[i]) || !data[i].siteinfo || data[i].siteinfo.username == $scope.user.username) {
                                        continue;
                                    }
                ...
                // 是否是用户自己的页面
                function isSelfPage() {
                    if (!isUserExist() || util.isEmptyObject(currentPage) || $scope.user.username != currentPage.username) {
                        return false;
                    }
                    return true;
                }
                ...
                function savePageContent(cb, errcb) {
                    ...
                    // 不能修改别人页面
                    if (!isUserExist() || util.isEmptyObject(currentPage) || !currentPage.isModify) {
                        cb && cb();
                        return;
                    }
                ...
                function initEditor() {
                    ...
                    function setDropFilePosition(cm, changeObj){
                        if (util.isEmptyObject(dropFiles)){
                            return;
                        }
