...
], function (app, markdownit, markdownwiki, storage, util, mdconf, dataSource, loading, homeHtmlContent, headerHtmlContent, footerHtmlContent, userHtmlContent, notfoundHtmlContent, serviceStopHtmlContent, crosHtmlContent) {
	...
	app.controller('mainController', [
		...
		function (
				...
				if (urlObj.username && urlObj.sitename) {
					...
					util.http("POST", config.apiUrlPrefix + "website/getDetailInfo", {
						...
						if (err.status) {
							switch (err.status) {
								case 404:
									errContent = notfoundHtmlContent;
									break;
								case 403:
									errContent = serviceStopHtmlContent;
									break;
								default:
									break;
							}
			...
			function initContentInfo() {
				...
				util.get(config.apiUrlPrefix + 'user/isLogin', {
